...
class CollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        } else if cellAttributesArray?.count == 1 {
            let currentLayoutAttributes: UICollectionViewLayoutAttributes = cellAttributesArray![0]
            var frame: CGRect? = currentLayoutAttributes.frame
            frame?.origin.x = 8
            frame?.origin.y = currentLayoutAttributes.frame.origin.y
            currentLayoutAttributes.frame = frame ?? CGRect.zero
